...
async function fetchVpicJson(url: string) {
  ...
  // vPIC occasionally returns HTML (rate limit / outage).
...
export async function GET(req: Request) {
  try {
    ...
    const url = `https://vpic.nhtsa.dot.gov/api/vehicles/GetMakesForVehicleModelYear/modelyear/${encodeURIComponent(
      year
    )}?format=json`;

    const j = await fetchVpicJson(url);

    const makes = (j?.Results || [])
      .map((x: any) => x?.MakeName)
      .filter(Boolean)
      .sort((a: string, b: string) => a.localeCompare(b));

    return NextResponse.json({ ok: true, year, makes });
  } catch (err: any) {
